...
{
    private static readonly FrozenDictionary<byte, string> TerminalNames = new Dictionary<byte, string>
    {
        { 3, "精算機" },
        { 4, "携帯型端末" },
        { 5, "車載端末" },
    ...
    private static readonly FrozenDictionary<int, string> RegionNames = new Dictionary<int, string>
    {
        { 0, "首都圏" },
        { 1, "中部圏" },
        { 2, "近畿圏" },
    ...
    public static string ConvertProcessString(byte process)
    {
        ...
        var name = ProcessNames.TryGetValue(processType, out var value) ? value : processType.ToString("X", CultureInfo.InvariantCulture);
    ...
    public static bool IsProcessOfSales(byte process) =>
        ConvertProcessType(process) is 70 or 72 or 73 or 74 or 75;

    public static bool IsProcessOfBus(byte process) =>
        ConvertProcessType(process) is 13 or 15 or 31 or 35;

    public static string ConvertRegionString(int region) =>
        RegionNames.TryGetValue(region, out var value) ? value : region.ToString("X", CultureInfo.InvariantCulture);
